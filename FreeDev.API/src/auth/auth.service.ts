...
export class AuthService {
  ...
  async validateUser(email: string, password: string): Promise<any> {
    const userFromModel: any = await this.userServ.findUserByEmail(email);
    ...
    const isHashMatchesPassword = await bcrypt.compare(
      password,
      userFromModel.passwordHash.toString(),
    );
  ...
  async findUserByEmail(email: string): Promise<any> {
    const userFromModel = await this.userServ.findUserByEmail(email);
