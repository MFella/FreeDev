import { FoldersStructure } from '../types/notes/foldersStructure';
import {
  Body,
  Controller,
  Get,
...
import { FoldersStructureQuery } from 'src/types/notes/foldersStructureQuery';
...
export class MailController {
  ...
  async sendDirectMessage(
    @Body() directMessageToSendDto: DirectMessageToSendDto,
    @Req() request: any,
  ): Promise<boolean> {
  ...
  @Post('indirect')
  @UseGuards(JwtAuthGuard)
  async sendIndirectMessage(
    @Body() indirectMessageToSendDto: IndirectMessageToSendDto,
    @Req() request: any,
  ): Promise<boolean> {
    return await this.mailService.tryToSaveIndirectMessage(
  ...
  async getMessageContent(
    @Query() contentMessageQuery: ContentMessageQuery,
    @Req() request,
  ): Promise<Partial<Mail>> {
    return await this.mailService.getMessageContent(
      contentMessageQuery?.messageId,
      request.user.userId,
    );
  }
  ...
  async updateMessageReadStatus(
    @Body() messageIdDto: { messageId: string },
  ): Promise<boolean> {
    return await this.mailService.updateMessageReadStatus(
  ...
  async moveMessageToFolder(
    @Body() moveMailDto: MoveMailDto,
    @Req() request: any,
  ): Promise<boolean> {
    return await this.mailService.moveMailToFolder(
      moveMailDto.mailId,
      moveMailDto.targetFolder,
      request.user.userId,
    );
  }
