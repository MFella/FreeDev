...
import {KindOfMailDropdownItem} from "../types/contacts/kindOfMailDropdownItem";
...
export class CreateMailComponent implements OnInit {
  ...
  private static readonly INVITATION_MESSAGE = 'Hi, I wanna be your friend. Cheers!';
  private static readonly DELETION_MESSAGE = 'Hi, I don\'t wanna be your friend anymore. Cya!'
  ...
  getReceiverTooltipValidationMessage(): string {
    ...
    if (receiverFormControl?.errors?.required) {
      validationMessage += FormInputKeys.RECEIVER[0].toUpperCase() + FormInputKeys.RECEIVER.slice(1) + ' is required';
    }
  ...
  getContentTooltipValidationMessage(): string {
    ...
    if (contentFormControl?.errors?.required) {
      validationMessage += FormInputKeys.CONTENT[0].toUpperCase() + FormInputKeys.CONTENT.slice(1) + ' is required';
    }

    if (contentFormControl?.errors?.minLength) {
      validationMessage += 'This field should be longer than ' + CreateMailComponent.CONTENT_FIELD_MIN_LENGTH + ' characters';
    }
    return validationMessage;
  }
  ...
  getTitleTooltipValidationMessage(): string {
    ...
    if (titleFormControl?.errors?.required) {
      validationMessage += FormInputKeys.TITLE[0].toUpperCase() + FormInputKeys.TITLE.slice(1) + ' is required';
    }
    return validationMessage;
  }
  ...
  selectedKindOfMailChanged(selectedKindOfMail: KindOfMailDropdownItem): void {
    this.mailForm.get(FormInputKeys.KIND_OF_MESSAGE)?.setValue(selectedKindOfMail);
    if (selectedKindOfMail.getKindOfMail() === KindOfMail.DELETION) {
      this.mailForm.get(FormInputKeys.CONTENT)?.setValue(CreateMailComponent.DELETION_MESSAGE);
    }

    if (selectedKindOfMail.getKindOfMail() === KindOfMail.INVITATION) {
      this.mailForm.get(FormInputKeys.CONTENT)?.setValue(CreateMailComponent.INVITATION_MESSAGE);
    }
  ...
  isKindOfJustMessage(): boolean {
    return this.mailForm.get(FormInputKeys.KIND_OF_MESSAGE)?.value === this.kindsOfMail[0];
  }
  ...
  private initForm(): void {
    ...
    this.mailForm = new FormGroup({
      ...
      [FormInputKeys.CONTENT]: new FormControl('', [Validators.required,
